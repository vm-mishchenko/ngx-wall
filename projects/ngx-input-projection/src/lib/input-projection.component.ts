...
export class InputProjectionComponent implements AfterContentInit, AfterViewInit, OnDestroy {
    ...
    ngAfterViewInit() {
        ...
        this.keyStream$.subscribe((event) => {
            ...
            if (event.keyCode === ENTER) {
                ...
                // it could be null in case when there are no item-renderer elements
                if (this.keyManager.activeItem) {
                    // todo: seems it stops work in case there are few input-projection element on the page
                    // most probably "scrollIntoView" method could be called twice
                    // skip for now, will fix it later if there will be a real usage case
                    this.keyManager.activeItem.scrollToMe();
                }
            }
        });
    }
    ...
    /**
     * Handle user click on item-renderer
     * */
    ...
    /**
     * When user click to item mark it as active,
     * so it become as starting point for  any further keyboard navigation.
     */
